...
namespace DataAccessLayer.Infrastructure
{
    ...
    {
        private static string GetTableName(BaseEntity obj)
        {
            return obj.GetType().GetCustomAttribute<TableName>().Text;
        }
        #region Insert
        public static SqlCommand
            BuildInsertCommand(BaseEntity item)
        {
        ...
        private static void GenerateInsertParameters(SqlCommand command, BaseEntity item)
        {
            foreach (PropertyInfo propriedade in item.GetType().GetProperties())
            {
                if (propriedade.Name != "ID")
                {
                    if (propriedade.GetValue(item) == "" || propriedade.GetValue(item) == null)
                        ...
                        if (propriedade.GetType().BaseType == typeof(BaseEntity))
                        command.Parameters.AddWithValue("@" + propriedade.Name, ((BaseEntity)propriedade.GetValue(item)).ID);
                    else
        ...
        private static string GetInsertFields(bool isParameters, BaseEntity obj)
        {
        ...
        #region Update
        public static SqlCommand BuildUpdateCommand(BaseEntity item)
        {
            StringBuilder builder = new StringBuilder();
        ...
        private static string GetUpdateFields(BaseEntity obj)
        {
            StringBuilder builder = new StringBuilder();
        ...
        private static void GenerateUpdateParameters(SqlCommand command, BaseEntity item)
        {
            foreach (PropertyInfo propriedade in item.GetType().GetProperties())
            {
        ...
        #region Delete
        public static SqlCommand BuildDeleteCommand(BaseEntity item)
        {
            SqlCommand command = new SqlCommand();
        ...
        #region Select
        public static SqlCommand BuildSelectCommand(BaseEntity item)
        {
            StringBuilder comando = new StringBuilder();
        ...
        private static void GenerateSelectWhere(SqlCommand command, BaseEntity item)
        {
            foreach (PropertyInfo propriedade in item.GetType().GetProperties())
            {
                try
